...
impl DFAQueryEngine {
    ...
    /// [`QueryDFA::find_limited`], and [`QueryDFA::for_each_match`]; the
    /// latter is the public entry point for existence checks or streaming
    /// consumption where materializing every match up front is wasteful.
    fn visit_with_dfa<'a, B, F>(
        json: &'a Value<'a>,
        dfa: &QueryDFA,
        mut f: F,
    ) -> ControlFlow<B>
